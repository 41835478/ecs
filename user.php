...
if ($action == 'default')
{
    ...
    if($info['is_validate'] == 0){
        $content = $_LANG['not_validated_alert'];
    ...
    $smarty->assign('user_notice', $_CFG['user_notice'].$content);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            if($is_validate == 0){
                $message .= "<br/>".$_LANG['not_validated'].
    "<a href='javascript:sendHashMail()' style='color:#006bd0;background:none;'>{$_LANG['resend_hash_mail']}</a>";
                show_message($message, array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, 'user.php'), 'info',false);
            }
            else
                show_message($message, array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, 'user.php'), 'info',true);
        }
        else
        {
...
elseif ($action == 'act_login')
{
    ...
    if ($user->login($username, $password))
    {
        ...
        $message = $_LANG['login_success'] . $ucdata;
        $id = $_SESSION['user_id'];
    ...
    "<a href='javascript:sendHashMail()' style='color:#006bd0;background:none;'>{$_LANG['resend_hash_mail']}</a>";
            ...
            show_message($message , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act,'user.php'), 'info');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
