...
elseif ($_REQUEST['act'] == 'toggle_auto_delivery')
{
    ...
    $goods_id       = intval($_POST['id']);
    $auto_delivery        = intval($_POST['val']);

    if ($exc->edit("auto_delivery = '$auto_delivery', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
...
//-- 修改精品推荐状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_shop_delivery')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $shop_delivery        = intval($_POST['val']);

    if ($exc->edit("shop_delivery = '$shop_delivery', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
