...
class paypal
{
   ...
   && in_array($_COOKIE['ECS']['preferred_currency'],$GLOBALS['allowed_currency'])){
            $currency_code = $_COOKIE['ECS']['preferred_currency'];
        }
        $data_amount = get_foreign_price($data_amount,$currency_code);
        ...
        else
        {
            ...
            while (!feof($fp))
            {
                ...
                if (strcmp($res, 'VERIFIED') == 0)
                {
                    ...
                    $data_amount = $GLOBALS['db']->getOne($sql);
                    if(get_foreign_price($data_amount,$payment_currency) != $payment_amount)
                    //if ($data_amount != $payment_amount)
                    {
                        fclose($fp);
                        return false;
                    }
                    
                    if(!in_array($payment_currency,$GLOBALS['allowed_currency']))
