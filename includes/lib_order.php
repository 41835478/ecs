 ...
 * ============================================================================
 * $Author: wangleisvn $
 * $Id: lib_order.php 16425 2009-07-03 05:17:20Z wangleisvn $
 */
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        $order['formated_goods_amount']   = price_format($order['goods_amount'], false,true);
        $order['formated_discount']       = price_format($order['discount'], false,true);
        $order['formated_tax']            = price_format($order['tax'], false,true);
        $order['formated_shipping_fee']   = price_format($order['shipping_fee'], false,true);
        $order['formated_insure_fee']     = price_format($order['insure_fee'], false,true);
        $order['formated_pay_fee']        = price_format($order['pay_fee'], false,true);
        $order['formated_pack_fee']       = price_format($order['pack_fee'], false,true);
        $order['formated_card_fee']       = price_format($order['card_fee'], false,true);
        $order['formated_total_fee']      = price_format($order['total_fee'], false,true);
        $order['formated_money_paid']     = price_format($order['money_paid'], false,true);
        $order['formated_bonus']          = price_format($order['bonus'], false,true);
        $order['formated_integral_money'] = price_format($order['integral_money'], false,true);
        $order['formated_surplus']        = price_format($order['surplus'], false,true);
        $order['formated_order_amount']   = price_format(abs($order['order_amount']), false,true);
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total['goods_price_formated']  = price_format($total['goods_price'], false,true);
    $total['market_price_formated'] = price_format($total['market_price'], false,true);
    $total['saving_formated']       = price_format($total['saving'], false,true);
    ...
    $total['discount_formated'] = price_format($total['discount'], false,true);
    ...
    $total['tax_formated'] = price_format($total['tax'], false,true);
    ...
    $total['pack_fee_formated'] = price_format($total['pack_fee'], false,true);
    ...
    $total['card_fee_formated'] = price_format($total['card_fee'], false,true);
    ...
    $total['bonus_formated'] = price_format($total['bonus'], false,true);
     ...
     if (!empty($order['bonus_kill']))
    {
        ...
        $total['bonus_kill_formated'] = price_format($total['bonus_kill'], false,true);
    }
    ...
    $total['shipping_fee_formated']    = price_format($total['shipping_fee'], false,true);
    $total['shipping_insure_formated'] = price_format($total['shipping_insure'], false,true);
    ...
    $total['surplus_formated'] = price_format($order['surplus'], false,true);
    ...
    $total['integral_formated'] = price_format($total['integral_money'], false,true);
    ...
    $total['pay_fee_formated'] = price_format($total['pay_fee'], false,true);

    $total['amount']           += $total['pay_fee']; // 订单总额累加上支付费用
    $total['amount_formated']  = price_format($total['amount'], false,true);
    ...
    $total['will_get_bonus']        = $order['extension_code'] == 'exchange_goods' ? 0 : price_format(get_total_bonus(), false,true);
    $total['formated_goods_price']  = price_format($total['goods_price'], false,true);
    $total['formated_market_price'] = price_format($total['market_price'], false,true);
    $total['formated_saving']       = price_format($total['saving'], false,true);
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    {
        $arr[$key]['formated_market_price'] = price_format($value['market_price'], false,true);
        $arr[$key]['formated_goods_price']  = price_format($value['goods_price'], false,true);
        $arr[$key]['formated_subtotal']     = price_format($value['subtotal'], false,true);
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['subtotal']     = price_format($row['goods_price'] * $row['goods_number'], false,true);
        $row['goods_price']  = price_format($row['goods_price'], false,true);
        $row['market_price'] = price_format($row['market_price'], false,true);
    ...
    $total['saving']       = price_format($total['market_price'] - $total['goods_price'], false,true);
    if ($total['market_price'] > 0)
    {
        $total['save_rate'] = $total['market_price'] ? round(($total['market_price'] - $total['goods_price']) *
        100 / $total['market_price']).'%' : 0;
    }
    $total['goods_price']  = price_format($total['goods_price'], false,true);
    $total['market_price'] = price_format($total['market_price'], false,true);
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    {
        case 0 :
            $sql = "SELECT goods_id, SUM(send_number) AS num, max(extension_code) as extension_code FROM " . $GLOBALS['ecs']->table('order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id ";
        break;

        case 1 :
            $sql = "SELECT goods_id, SUM(goods_number) AS num, max(extension_code) as extension_code FROM " . $GLOBALS['ecs']->table('order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id ";
        break;
    }
